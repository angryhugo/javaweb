...
/**
 * Servlet implementation class QueryCarForS
 */
@WebServlet("/QueryCarForS")
public class QueryCarForS extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<form action='QueryCarForS'><p class='style2'>颜色：<select name='colorselect' size='1' id='select'><option value='black'>黑色</option><option value='white'>白色</option><option value='silvery'>银色</option><option value='else'>其他</option><option value='all'  selected='selected'>不限</option></select>");
		...
		if (color != null && manu != null && year != null && price != null) {
			...
			try {
				s = search.search_vehicle_forS(sql);
			} catch (Exception e) {
