...
public class DelVehicle extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<BODY>");
		out.println("<p align='center' class='style1'>删除车辆信息</p>");
		out.println("<form  id=\"form1\" name=\"form1\" action='DelVehicle'><p class='style2'>请输入要删除的车辆编号：<input name='name' type='text' size='12'/><input type='button' value='删除' onclick='checkDelnull();'/></p></form>");
		if(number!=null)
		{
			byte b[]=number.getBytes("ISO-8859-1");//必须先做转换！！！
			number=new String(b);
			String sql="delete from vehicle where Vnum='"+number+"'";
			try {
				if(delete.doIDU(sql))
